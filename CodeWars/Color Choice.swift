...
func checkChoose(_ m: Int, _ n: Int) -> Int {
    ...
    // We set our initial return to 1
    var result = 1
    
    // For each number between 0 and n
    for i in 0...n {
        
        // We evaluate whether or not the reusult is == m at the top of the loop and return i, ending the loop at the lowest number that meets the criteria
        if result == m {
            return i
        }
        
        // If result is not equal to m, we set it equal to the current value * the quotient of (n - i) over (i + 1)
        
        // There's probably a mathematical proof for why this is the case, but I have to plead ignorance.
        result = result * (n - i) / (i + 1)
    }
    
    // If we make it through the whole for loop without a result, we return -1
    return -1
}
